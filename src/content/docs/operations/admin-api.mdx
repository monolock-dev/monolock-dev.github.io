...
gives lock introspection, force-release, and removal of waiters.

<Aside type="caution" title="The bind address is the access control">
  The API has no authentication of its own. Keep it on a loopback address —
  the server writes a warning to the log at startup if it is not one. From
...
the same [glob language](/operations/acl/#glob-language) as the
...
waiter queue in first-in, first-out (FIFO) sequence. `identity` is empty if
[mutual TLS (mTLS)](/operations/tls/#client-identity) is not on.
