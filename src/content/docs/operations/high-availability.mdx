...
- Two identical monolock instances on two machines, with the same
  [configuration](/operations/configuration/), certificates, and
...
- NTP (Network Time Protocol) on the two machines. This is the only
  requirement that connects them.
...
- **The fence.** When the failover takes the virtual IP (VIP) address
  away, the old sessions
...
- **The queues.** The first-in, first-out (FIFO) sequence forms again in
  the sequence of the
...
<Aside type="caution" title="Two cautions">
  ...
  - The switch must break the connections to the old server. Many load
    ...
    the old process — this is the fence. With DNS, the time to live (TTL)
    of the record also adds to `D`.
</Aside>
