...
- **One connection is one claim on one lock.** The connection *is* the lease.
  ...
  can lose.
- **Named locks, FIFO (first in, first out) waiters.** Lock names are raw UTF-8 strings with a
...
- **Connection-scoped ownership.** After a controlled stop of the owner, the
  ...
  full lease.
- **Client-selected leases, RTT (round-trip time)-aware heartbeats.** Each client selects the
...
- [TLS and mutual TLS (mTLS)](/operations/tls/) with certificate rotation on `SIGHUP`
- [ACL (access-control list) authorization](/operations/acl/): mTLS identity → lock name glob rules
