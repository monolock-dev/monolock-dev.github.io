...
first-in, first-out (FIFO) queue. After a controlled shutdown, the subsequent client takes the lock
...
server: milliseconds on the same machine, seconds across an unreliable
...
stop the session. The round-trip time (RTT) correction only makes the
interval *smaller*. On a
...
| Process killed, socket reset by the OS | immediate | the TCP reset (RST) reaches the server |
| Hang, garbage-collection (GC) pause, network partition | ≤ the owner's lease | the server must wait for the full lease of silence |
