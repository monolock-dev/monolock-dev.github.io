...
does not give *safety*. A garbage-collection (GC) cycle, a closed laptop lid, or a network problem
...
write, a compare-and-swap (CAS), an `if token >= stored_token` check. The resource must reject
...
machine is not behind the clock of the old machine. Synchronisation with
the Network Time Protocol (NTP) gives this.
