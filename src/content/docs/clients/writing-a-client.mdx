...
- decrease the interval by two times the round-trip time (RTT), smoothed
  with an exponentially weighted moving average (EWMA, alpha 0.2); do
...
new connection, at the back of the first-in, first-out (FIFO) queue. There is no silent
re-acquisition.
